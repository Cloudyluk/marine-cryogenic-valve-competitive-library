...
describe('competitive coverage', () => {
  ...
  it('retains official model-level specifications recovered from competitor datasheets', () => {
    ...
    const baker33000 = valveSeries.find((item) => item.id === 'baker-masoneilan-33000')
    ...
    const gwcFloating = valveSeries.find((item) => item.id === 'gwc-floating-ball')
    const habonimC74 = valveSeries.find((item) => item.id === 'habonim-c74')
    const habonimC52 = valveSeries.find((item) => item.id === 'habonim-c52')
    const furuiLng = valveSeries.find((item) => item.id === 'furui-low-temp-valves')
    const raysTopEntry = valveSeries.find((item) => item.id === 'rays-top-entry-cryo-floating-ball')
    const herose03841 = valveSeries.find((item) => item.id === 'herose-03841')
    const brayMccannalok = valveSeries.find((item) => item.id === 'bray-mccannalok-cryo')
    const ombC230Plug = valveSeries.find((item) => item.id === 'omb-c230-cryo-plug')
    const ombC230Cage = valveSeries.find((item) => item.id === 'omb-c230-cryo-cage')
    const newayGate = valveSeries.find((item) => item.id === 'neway-cryo-gate')
    const habonimC82 = valveSeries.find((item) => item.id === 'habonim-c82')
    const habonimC47Bd = valveSeries.find((item) => item.id === 'habonim-c47-bd')
    ...
    expect(baker33000).toMatchObject({ pressure: 'ASME Class 150–600（公开产品页）', size: '3–48 in / DN80–1200（公开产品页）' })
    expect(baker21000).toMatchObject({ pressure: '序列号牌标注最大允许压力（官方 EB 附录）', size: '3/4–8 in / DN20–200（21000 系列公开产品页）' })
    ...
    expect(baker21000).toMatchObject({ minTemperature: -200, connection: ['法兰式', 'BW'] })
    expect(baker21000?.construction).toEqual(expect.arrayContaining(['加长阀盖（EB）', '加长阀芯杆']))
    ...
    expect(brayMccannalok).toMatchObject({ minTemperature: -196, pressure: 'ASME Class 150–300；EN 1092 PN10–40（官方产品页）', size: 'NPS 3–24 / DN80–600（官方产品页）' })
    expect(brayMccannalok?.classSocieties).toEqual(expect.arrayContaining(['ABS', 'Bureau Veritas', 'CCS', 'DNV']))
    expect(ombC230Plug).toMatchObject({ minTemperature: -196, pressure: 'ASME Class 150–2500（官方产品页）', size: 'DN15–50（页面同时标注 1/2–6 in，待订单确认）' })
    expect(ombC230Plug?.standards).toEqual(expect.arrayContaining(['ASME B16.34', 'PED 2014/68/EU']))
    expect(ombC230Cage).toMatchObject({ minTemperature: -196, pressure: 'ASME Class 150–2500（官方产品页）', size: 'DN15–50（页面同时标注 1/2–6 in，待订单确认）' })
    expect(ombC230Cage?.accessories).toEqual(expect.arrayContaining(['Vogt AVL1 / AVL2 气动执行机构']))
    expect(newayGate).toMatchObject({ minTemperature: -196, pressure: 'ASME Class 150–2500 / PN16–420（官网产品页）', size: 'NPS 1/2–52 / DN15–1300（官网产品页）' })
    expect(newayGate?.connection).toEqual(expect.arrayContaining(['RF', 'BW', 'RTJ']))
    expect(habonimC82).toMatchObject({ pressure: 'Class 300 / 50 bar / 750 psi（官方产品页）', size: '2–12 in / DN50–300（官方产品页）' })
    expect(habonimC47Bd).toMatchObject({ pressure: 'Class 150/300 / 20/50 bar / 290/750 psi（官方产品页）', size: '1/4–8 in / DN8–200（官方产品页）' })
